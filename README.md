...
The line schedules carry a **forward line-load plan** (`campaigns`): the balance-to-produce of every open order as ~20 t campaign coils inserted coil by coil, forward in time, around the recorded coils over a 10-day horizon (each step as early as possible after the previous one plus a transfer time, releases paced by the CGL takt, an order's coils consecutive, same-family orders adjacent, changeover minutes between families, WIP coils first). The generator asserts the EST precedence rule for every recorded and planned coil. Routes & Schedules shows the full line load (one colour per order) and the selected order's own schedule (one colour per coil) as two gantts with an EST self-check; the **Coil Plan** page is the report view of the same load (one row per coil in sequence, route chips, planned window, expandable operation schedule per step, order / line / kind filters); reroutes shift a coil's later steps when needed so the rule holds; the dashboard's on-track / behind counts include the campaign coils.

More than one order runs through the live flows: every running line carries a different sales order, and three finished coils wait in the FG yard (two colour-coated coils with a pending lab result plus the hero coil), so the Quality → Packing → Dispatch steps of Scenario 1 can be repeated on a second order. The certificate selector on Quality and the coil selector on Packing pick the coil.
