...
if ua and ub:
    ...
    for u, new, old in ((ua, SWAP_B, SWAP_A), (ub, SWAP_A, SWAP_B)):
        u["allocatedTo"] = new; u["swapped"] = dict(at=iso(AT(-2, 15, 20)), fromItem=old, toItem=new, by="PPC — A. Bhosale", reason="Earlier RDD on " + new + "; identical spec (DX51D+Z 0.80×1250 Z120)")
        T["edges"] = [e for e in T["edges"] if not (e[0] == "allocatedTo" and e[1] == u["id"])] + [("allocatedTo", u["id"], new)]
        T["allocations"].append(dict(id=f"ALC-SW-{u['id'][-4:]}", unitId=u["id"], itemId=new, previousItemId=old, at=iso(AT(-2, 15, 20)), by="PPC — A. Bhosale", ppcApproval="APPROVED", qcApproval="APPROVED", status="ACTIVE", reason="Batch swap between sales orders"))
    swap_event = dict(at=iso(AT(-2, 15, 20)), unitA=ua["id"], unitB=ub["id"], itemA=SWAP_A, itemB=SWAP_B, by="PPC — A. Bhosale", reason="SO " + SWAP_B + " has the earlier RDD (" + by_item[SWAP_B]["reqDate"] + "); both coils meet the identical TDC",
...
def write():
    ...
    with open(os.path.join(ROOT, "data", "khp-meta.js"), "w", encoding="utf-8") as f:
        f.write("window.KHP_META = " + json.dumps(dict(asOf=iso(ASOF), asOfLabel=ASOF.strftime("%a %d %b %Y %H:%M"), shift="A", plant=A.PLANT["name"], hero=KHP["meta"]["hero"]), ensure_ascii=False) + ";\n")
    ...
    lines = ["# Khopoli NextGen MES — demo dataset (data dictionary)", "",
             ...
             "## Id conventions", "- Sales order `42130900nn`, item `42130900nn/10`, schedule line `42130900nn/10/0001`, TDC `TDC-42130900nn-10`, FP order `FPM-…`, MES production order `PO-KHP-" + YM + "-nnnn`.",
             "- Material units by product: `HRC-VJ|DL-" + YM_PREV + "-nnnn` (HR coil, source plant), `HPO-` (pickled), `CRF-` (full hard), `GIC-`/`GLC-` (galvanized / galvalume), `PPG-`/`PPL-` (colour coated), `SLC-` (slit child), `PK-` (pack). Suffix `-KHP-" + YM + "-` = plant + year/month batch identifier.",
             "- Transactions: `PDI-<line>-nnnn`, `PDO-<line>-nnnn`, `PC-<line>-nnnn` (confirmation), `DLY-`, `DEF-" + YM + "-`, `UD-`, `DGS-` (downgrade suggestion), `TC-<unit>` (certificate), `HLD-`, `SLP-`, `DSP-`, `PBILL-`.",
